...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        pdb.set_trace()
        if args.distributed:
            # 使shuffle操作能够在多个epoch中正常工作。 否则dataloader迭代器产生的数据将始终使用相同的顺序
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (coord, feat, target, offset) in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
        ...
        # print(coord.shape, feat.shape, target.shape, offset.shape)
        # torch.Size([110327, 3]) torch.Size([110327, 3]) torch.Size([110327]) torch.Size([4])
